...
always @(*) begin
    if(sm_stall)begin
        sm_stall = (y_buff_v)? 1'b1 : 1'b0;
    end else begin
        sm_stall = (acc_clear_nxt)? y_buff_v : 1'b0;
    end
end
...
//-------------------------------------------------------------------------------------------------------------//
    ...
    assign data_WE            = {data_write , data_write , data_write , data_write}; 
            ...
            if(tap_read_done)begin
                if(tap_addr_rbuff < tap_para_base)begin
                    case(tap_addr_rbuff[(pCONFIG_REF-1):0])
                        ap_base         : tap_data_rbuff_nxt = {{(pDATA_WIDTH-3){1'b0}} , fir_state};
